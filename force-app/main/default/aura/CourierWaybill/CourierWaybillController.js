({
    ...
    findAddress : function(component, event, helper) {
        ...
        let account_Id = component.get('v.workOrder.Account__c');
    ...
    requestDelivery : function(component, event, helper) {
        let button = event.getSource();
        button.set('v.disabled', true);
        let workOrder_Id = component.get('v.workOrder.Id');
        let suburb_Id = component.find('addressTable').getSelectedRows()[0];
        let action = component.get('c.generateWaybill');

        action.setParams({workOrderId : workOrder_Id, suburb : suburb_Id});
        action.setCallback(this, $A.getCallback(function (response) {
            let state = response.getState();
            if (state === "SUCCESS") {
